 ...
 *
 *	Break/fix
 *  ~ Added RowLimit support to roboCAML.QueryOptions()
 *	~ Fixed <ViewAttributes> to work with ClientOM. e.g.: <View Scope=\'RecursiveAll\'>
 *	~ Fixed <OrderBy> to work with ClientOM. e.g.: <OrderBy> is now contained within the <Query> node. OOPS!
 *	~ Deprecated: roboCAML.AddCamlTags();
 * ~	Fixed small bug in roboCAML.QueryOptions(); ViewAttributes now respects false being passed in.
 */
	...
	return {
		...
		QueryOptions: function( opt, internalUsage ) {
			...
			*******************************************************/
			...
			for ( var prop in opt ) {
				if ( opt.hasOwnProperty( prop ) ) {
					//Cache original value for property access later.
					var _prop = prop;
					...
					switch ( prop ) {
						...
						case "Folder": //Should this value have a trailing "/" ????? <---- Doesn't matter. :~)
							//Folder should contain: /URL/ListName/TopLevelFolder/SubFolder
