...
var roboCAML = (function( $ ) {
	...
	/*  These vars may cause issues w/ being cached.  Verify when testing */
			...
			opt.batchCMD = opt.batchCMD || "Update";
					...
					if ( opt.hasOwnProperty( "IDs" ) ) {
						...
						for ( i=0; i < loopLength; i++ ) {
							...
							var currObj = opt.updates[ i ],
								...
								crudOp = currObj.batchCMD || opt.batchCMD;
		...
		OrderBy: function( arr ) {
			...
			if ( typeof arr !== "undefined" && arr.length ) {
					...
					while ( i < loopLength ) {
						//console.log(arr[i]);
						orderBy += "<FieldRef Name='" + arr[ i ] + "' Ascending='" + CorrectCAML( arr[ ( i + 1 ) ] ) + "'/>";
						i = i+2;
					}
					orderBy += "</OrderBy>";
					return orderBy;
...
console.log( 
	roboCAML.BatchCMD({
		...
		updates: [
			{
				ID: "1", //Pass a string, or......
				//Static Column Name, Value
				valuePairs: ["Rank", "Numero Uno", "Description", "Some Notes", "Col3", "Update3"]
			},
			{
				ID: 2, //Pass a number, doesn't matter
				valuePairs: ["Col1", "1"]
			},
			{
				ID: 3,
...
console.log(
	roboCAML.Query({
		...
		//webURL: "http://YourURL",
		//Mandatory
		listName: "Position Descriptions",
		//Mandatory
		config: [
			{
				//Valid ops: Eq, Neq, Gt, Geq, Lt, Leq, IsNull, IsNotNull, BeginsWith, Contains, DateRangesOverlap, Includes				
				//Mandatory
				op: "!=",
